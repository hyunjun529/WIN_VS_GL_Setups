...
int main(int, char**)
{
	...
	// hidden
	GLFWwindow *window_hidden;
	window_hidden = glfwCreateWindow(400, 400, "hidden", NULL, NULL);
	if (!window_hidden)
	{
		glfwTerminate();
	}
	glfwMakeContextCurrent(window_hidden);
	glfwWindowHint(GLFW_CONTEXT_VERSION_MAJOR, 2);
	glfwWindowHint(GLFW_CONTEXT_VERSION_MINOR, 0);
	...
	while (!glfwWindowShouldClose(window))
	{
		...
		glfwGetFramebufferSize(window_hidden, &width, &height);
		...
		glfwSwapBuffers(window_hidden);
		glfwPollEvents();

		glReadPixels(0, 0, width, height, GL_RGBA, GL_UNSIGNED_INT, data);
		...
		if (show_hidden) {
			if (!glfwGetWindowAttrib(window_hidden, GLFW_VISIBLE)) {
				glfwShowWindow(window_hidden);
			}
			else {
				glfwHideWindow(window_hidden);
			}
